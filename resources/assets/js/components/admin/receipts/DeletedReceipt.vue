<template>
        <tr>
            ...
            <td>
                <span class="pointer" @click="restoreReceipt"> <i class="fa fa-undo text-primary"  title="restore"></i> </span>
...
import moment from 'moment';
    ...
    export default {
        ...
        mounted() {
                ...
                eventHub.$on(`select-${this.id}`, this.selectAll);              
                eventHub.$on(`deselect-${this.id}`, this.deselectAll);
        },
        methods: {
            restore(){
            ...
            restoreReceipt(){
                eventHub.$emit('restore-receipt', this.receipt.id);
            },
            selectAll(){
