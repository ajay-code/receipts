...
import mixin from '../mixins/Receipts';
    ...
    export default {
      ...
      components: {
        ...
        methods: {
            reload(page){
              axios.get(`/api/receipts?search=${this.search}&records=${this.records}&page=${page}`).then( res => {
            ...
            reloadFromFirstPage(){
              axios.get(`/api/receipts?search=${this.search}&records=${this.records}`).then( res => {
                  this.receipts = res.data.data;
                  this.updatePageInfo(res.data);
                }).catch( err => {
                  alert(err);
